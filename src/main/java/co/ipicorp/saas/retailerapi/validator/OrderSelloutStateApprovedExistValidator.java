...
@Component
public class OrderSelloutStateApprovedExistValidator extends AbstractFormValidator {
	...
	private boolean validateById(SelloutIdForm selloutIdForm, Errors errors) {
		...
		if ( selloutIdForm.getSelloutId() != null ) {
	    	...
	    	if ( !orderSellout.getOrderStatus().equals(OrderSelloutStatus.APPROVED.toString())){
	    		errors.reject(ErrorCode.APP_2105_ORDER_SELLOUT_STATE_NOT_EXACTLY,
						new Object[] { "Order Sellout Id", selloutIdForm.getSelloutId(), OrderSelloutStatus.APPROVED.toString() },
